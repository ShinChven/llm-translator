...
Prerelease tags such as `v0.2.0-beta.1` create a GitHub prerelease. `package.json` and `package-lock.json` carry the full prerelease version, while Chrome's `manifest.json` uses the numeric core version (`0.2.0`).
